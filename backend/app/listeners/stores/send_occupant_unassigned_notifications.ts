...
/**
 * Listener pour la dissociation d'un occupant d'un magasin
 * Envoie un email à tous les actor managers de l'acteur dissocié
 */
export default class SendOccupantUnassignedNotifications {
  async handle(payload: OccupantUnassignedPayload) {
    try {
      logger.info(
        `📧 [Background] Envoi notifications dissociation occupant ${payload.actor.fullName} du magasin ${payload.store.name}`
      )
      ...
      const emailPromises = actorManagers.map(async (manager) => {
        ...
        return StoreEmailService.sendOccupantUnassignedEmail(
          manager.email,
          managerName,
          {
      ...
      logger.info(
        `✅ [Background] Notifications dissociation occupant envoyées: ${successCount} succès, ${failureCount} échecs`
      )
    } catch (error) {
      logger.error(
        `❌ [Background] Erreur envoi notifications dissociation occupant ${payload.actor.fullName}:`,
        error
      )
    }
  }
