...
function Calendar({
  ...
  const Dropdown = React.useCallback(
    ({
      ...
      const currentOptions = options?.filter((option) => !option.disabled);
        ...
        >
          ...
          <SelectContent position="popper" align="center">
            <ScrollArea className="max-h-50">
              {currentOptions?.map(({ value, label, disabled }, id) => {
                return (
                  <SelectItem
                    key={`${value}-${id}`}
                    value={value?.toString()}
  ...
  return (
    <DayPicker
      ...
      hideNavigation={hideNavigation} // Ensure navigation is hidden
