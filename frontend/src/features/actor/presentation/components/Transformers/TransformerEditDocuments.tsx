...
export default function TransformerEditDocuments() {
  ...
  // Récupérer les informations du transformateur
  const { actor } = useGetActorById(actorId || "");

  // Construire le nom complet du transformateur
  const transformerName = actor
  ...
  useEffect(() => {
    const loadDocuments = async () => {
      ...
      try {
        ...
        const transformerDocs: IFileValue[] = [];
        ...
        for (const doc of result.documents) {
          ...
          if (transformerDocTypes.has(docType)) {
            transformerDocs.push(fileValue);
  ...
  const handleSubmit = useCallback(
    async (data: Step3DocumentsData) => {
      ...
      try {
        ...
        router.replace(`/actors/transformers/view?entityId=${actorId}`);
      } catch (error) {
  ...
  const handleCancel = useCallback(() => {
    if (!actorId) return;
    router.push(`/actors/transformers/view?entityId=${actorId}`);
  }, [actorId, router]);
  ...
  const headerContent = (
    <div className="flex items-center space-x-3">
      <h1 className="text-xl font-medium text-gray-900">
        {t("transformer.documentsPageTitle")}
      </h1>
    </div>
  );
  ...
  return (
    <TransformerFormLayout
      ...
      title={t("transformer.documentsTitle", { name: transformerName })}
    ...
    >
      ...
      >
        {isLoadingDocuments ? (
          ...
          <Form {...form}>
            <form className="space-y-8" id="transformer-documents-form">
              {/* Section 1: Documents transformateur obligatoires */}
              <div className="lg:w-2/3">
                <FormDocumentUploadWithOption
                  ...
                  placeholder={t("transformer.documentsPlaceholder")}
                  maxSizeMB={2}
                  defaultFiles={form.getValues("documents") || []}
                  required
