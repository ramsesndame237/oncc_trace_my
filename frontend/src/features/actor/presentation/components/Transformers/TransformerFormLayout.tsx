...
interface TransformerFormLayoutProps {
...
export default function TransformerFormLayout({
  children,
  className,
  onHandleCancel,
  title,
}: TransformerFormLayoutProps) {
  ...
  const pageTitle = title || (isEditMode ? t("transformer.editTransformer") : t("transformer.createTransformer"));

  const handleCancel = () => {
    if (hasUnsavedChanges) {
      const confirmLeave = window.confirm(
        t("transformer.unsavedChangesWarning")
      );
    ...
    if (editOffline) {
      ...
      router.replace("/actors/transformers");
    }
  };
    ...
    <div className="min-h-screen bg-[#f8f9fa]">
      <PageHeader
        ...
        rightContent={[
          ...
          >
            <Icon name="X" />
            <span className="hidden lg:block">{t("transformer.cancelButton")}</span>
          </Button>,
        ]}
      />
