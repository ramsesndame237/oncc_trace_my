...
import type { CollectionByQuality } from "@/features/actor/domain/production.types";
import { useActorStore } from "@/features/actor/infrastructure/store/actorStore";
import type { ColumnDef } from "@tanstack/react-table";
import { useEffect, useState } from "react";
import { useTranslation } from "react-i18next";
import type { OpaCollectionsResponse } from "@/features/actor/domain/production.types";

interface OpaCollectionsTableProps {
  opaId: string;
  className?: string;
}

export const OpaCollectionsTable: React.FC<
  OpaCollectionsTableProps
> = ({ opaId, className }) => {
  const { t } = useTranslation(["actor", "common"]);
  const { getOpaCollections } = useActorStore();
  const [data, setData] = useState<OpaCollectionsResponse | null>(null);
  ...
  const loadCollections = async () => {
    ...
    setError(null);
    try {
      const result = await getOpaCollections(opaId);
      setData(result);
    } catch (err) {
  ...
  // Colonnes du tableau
  const columns: ColumnDef<CollectionByQuality>[] = [
    {
      accessorKey: "quality",
      header: () => (
  ...
  return (
    <div className={className}>
      ...
      <DataTable
        ...
        emptyMessage={t("opa.noCollections")}
      />
