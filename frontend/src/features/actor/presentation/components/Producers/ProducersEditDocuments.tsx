...
export default function ProducersEditDocuments() {
  const { t } = useTranslation(["actor", "common"]);
  const router = useRouter();
  const searchParams = useSearchParams();
  const actorId = searchParams.get("entityId");

  const { opaDocuments, landOPADocuments, complementDocuments } =
    useOPAOptions();
  ...
  useEffect(() => {
    const loadDocuments = async () => {
      ...
      try {
        ...
        const opaDocTypes = new Set<string>(opaDocuments.map((d) => d.value));
        ...
        for (const doc of result.documents) {
          ...
          if (opaDocTypes.has(docType)) {
            opaDocs.push(fileValue);
  ...
  const handleSubmit = useCallback(
    async (data: Step4DocumentsData) => {
      ...
      try {
        ...
        router.replace(`/actors/producers/view?entityId=${actorId}`);
      } catch (error) {
  ...
  const handleCancel = useCallback(() => {
    if (!actorId) return;
    router.push(`/actors/producers/view?entityId=${actorId}`);
  }, [actorId, router]);
  ...
  const headerContent = (
    <div className="flex items-center space-x-3">
      <h1 className="text-xl font-medium text-gray-900">
        {t("producers.documentsPageTitle")}
      </h1>
    </div>
  );
  ...
  return (
    <ProducersFormLayout
      ...
      title={t("producers.documentsTitle", { name: opaName })}
    ...
    >
      ...
      >
        {isLoadingDocuments ? (
          ...
          <Form {...form}>
            <form className="space-y-8" id="opa-documents-form">
              {/* Section 1: Documents de l'OPA */}
              <div className="lg:w-2/3">
                <FormDocumentUploadWithOption
                  ...
                  placeholder={t("producers.documentsPlaceholder")}
                  maxSizeMB={2}
                  defaultFiles={form.getValues("opaDocuments") || []}
                  required
              ...
              <div className="lg:w-2/3">
                <FormDocumentUploadWithOption
                  ...
                  label={t("producers.landProofDocuments")}
                  options={landOPADocuments}
                  placeholder={t("producers.landProofDocumentsPlaceholder")}
                  labelButton={t("documents.uploadButton")}
              ...
              <div className="lg:w-2/3">
                <FormDocumentUploadWithOption
                  ...
                  label={t("producers.complementaryDocuments")}
                  options={complementDocuments}
                  placeholder={t("producers.complementaryDocumentsPlaceholder")}
                  labelButton={t("documents.uploadButton")}
