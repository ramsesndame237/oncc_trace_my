...
interface BuyerFormLayoutProps {
...
export default function BuyerFormLayout({
  ...
  const pageTitle = title || (isEditMode ? t("buyer.editBuyer") : t("buyer.createBuyer"));

  const handleCancel = () => {
    if (hasUnsavedChanges) {
      const confirmLeave = window.confirm(
        t("buyer.unsavedChangesWarning")
      );
    ...
    <div className="min-h-screen bg-[#f8f9fa]">
      <PageHeader
        ...
        rightContent={[
          ...
          >
            <Icon name="X" />
            <span className="hidden lg:block">{t("buyer.cancelButton")}</span>
          </Button>,
        ]}
      />
