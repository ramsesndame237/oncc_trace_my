...
import { useExporterFormStore } from "@/features/actor/infrastructure/store/exporterFormStore";
import ExportersFormLayout from "@/features/actor/presentation/components/Exporters/ExportersFormLayout";
import { useExporterFormNavigation } from "@/features/actor/presentation/hooks/useExporterFormNavigation";
import type { ColumnDef, PaginationState } from "@tanstack/react-table";
import { useCallback, useEffect, useMemo, useState } from "react";
import { useTranslation } from "react-i18next";

export default function ExporterEditStep3() {
  ...
  // Charger les buyers disponibles (excluant ceux déjà affectés à d'autres exportateurs)
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Extraire les IDs des buyers déjà associés à des exportateurs
        const assignedBuyerIds = allRelations
          .map((rel) => rel.buyerServerId || rel.buyerLocalId)
          .filter((id): id is string => Boolean(id));

        // Filtrer pour afficher uniquement les buyers non associés à un exportateur
        const availableBuyers = allActiveBuyers.filter((buyer) => {
          const buyerId = buyer.serverId || buyer.localId;
          if (!buyerId) return false;

          // Si le buyer est déjà associé à un exportateur, NE PAS l'afficher
          if (assignedBuyerIds.includes(buyerId)) return false;
        ...
        // Récupérer les IDs déjà sélectionnés dans le formulaire
        const formBuyerIds = formData.step3?.selectedBuyerIds || [];

        // Filtrer les IDs du formulaire pour ne garder que ceux qui sont disponibles
        const availableBuyerIds = availableBuyers
          .map((b) => b.serverId || b.localId)
          .filter((id): id is string => Boolean(id));

        const validFormBuyerIds = formBuyerIds.filter((id) =>
          availableBuyerIds.includes(id)
        );

        // Initialiser avec les IDs du formulaire qui sont disponibles
        setSelectedBuyerIds(new Set(validFormBuyerIds));

        // Mettre à jour le formulaire si des IDs ont été filtrés
        if (
          JSON.stringify(validFormBuyerIds.sort()) !==
          JSON.stringify(formBuyerIds.sort())
        ) {
          updateStep3Data({ selectedBuyerIds: validFormBuyerIds });
        }
  ...
  useEffect(() => {
    ...
    const currentIds = formData.step3?.selectedBuyerIds || [];
    ...
    if (isInitialized) {
      ...
      if (JSON.stringify(ids.sort()) !== JSON.stringify(currentIds.sort())) {
        updateStep3Data({ selectedBuyerIds: ids });
        saveProgress();
      }
    }
  }, [
      ...
      >
        ...
        <div className="mb-6">
          <p className="text-sm text-muted-foreground">
            {t("buyer.create.selectBuyersDescription")}
          </p>
        </div>
        ...
        <div className="mb-6">
          <div className="relative">
            ...
            <Input
              type="text"
              placeholder={t("buyer.create.searchPlaceholder")}
              value={searchQuery}
