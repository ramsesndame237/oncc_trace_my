...
import {
  createStep1ExporterInfoSchemaWithConditional,
  type Step1ExporterInfoData,
} from "../../schemas/exporter-validation-schemas";
import ExportersFormLayout from "./ExportersFormLayout";

export default function ExporterEditStep1() {
  ...
  const { navigateToNext, handleCancel } = useExporterFormNavigation();
  ...
  // Form setup
  const form = useForm({
    resolver: zodResolver(createStep1ExporterInfoSchemaWithConditional(t)),
    defaultValues: {
      ...
      metadata: {
        professionalNumber: formData.step1?.metadata?.professionalNumber || "",
        rccmNumber: formData.step1?.metadata?.rccmNumber || "",
        exporterCode: formData.step1?.metadata?.exporterCode || "",
      },
      ...
      existenceDeclarationCode:
        String(formData.step1?.existenceDeclarationCode) || "",
      ...
      if (entityId && editOffline) {
        ...
        try {
          ...
          if (pendingOperation && pendingOperation.payload) {
            ...
            const step1Data: Step1ExporterInfoData = {
              ...
              metadata: {
                professionalNumber:
                  (metadata.professionalNumber as string) || "",
                rccmNumber: (metadata.rccmNumber as string) || "",
                exporterCode: (metadata.exporterCode as string) || "",
              },
  ...
  useEffect(() => {
    const subscription = form.watch((data) => {
      updateStep1Data(data as Step1ExporterInfoData);
  ...
  return (
    <ExportersFormLayout className="lg:flex items-start lg:space-x-4">
      ...
      >
        ...
        <Form {...form}>
          <form className="space-y-8" id="exporter-step1-form">
            {/* Nom */}
