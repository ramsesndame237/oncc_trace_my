...
import type { ForgotUsernameRequest } from "../../domain/types/request";
import type { ForgotUsernameResponse } from "../../domain/types/response";

export interface ForgotUsernameInput {
  email: string;
}

export interface IForgotUsernameUseCase {
  execute(
...
@injectable()
export class ForgotUsernameUseCase implements IForgotUsernameUseCase {
  constructor(
  ...
  async execute(
    ...
    try {
      ...
      const emailData: ForgotUsernameRequest = {
        email: input.email.trim().toLowerCase(),
      };

      // Appel du repository injecté
      const result = await this.authRepository.forgotUsername(emailData);
