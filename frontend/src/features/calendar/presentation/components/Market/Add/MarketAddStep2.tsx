...
import { useMarketAddFormStore } from "@/features/calendar/infrastructure/store/marketAddFormStore";
import { useMarketAddFormNavigation } from "@/features/calendar/presentation/hooks/useMarketAddFormNavigation";
import {
  createStep2Schema,
  type Step2Data,
} from "@/features/calendar/presentation/schemas/market-validation-schemas";
...
import { MarketFormLayout } from "../MarketFormLayout";

export function MarketAddStep2() {
  ...
  const { handleFinish } = useMarketAddFormNavigation();

  // State pour stocker le nom de l'OPA
  ...
  const { opaId, startDate, endDate, locationCode, location, eventTime } =
    formData.step1;
  ...
  // Charger le nom de l'OPA depuis IndexedDB
  useEffect(() => {
  ...
  const changeHref = "/calendars/market/create/informations";

  // Header content
  const headerContent = (
    <div className="flex items-center space-x-3">
      <h1 className="text-xl font-medium text-gray-900">
        {t("calendar:forms.marketAdd.step2.marketInformation")}
      </h1>
    </div>
  );
  ...
  return (
    <MarketFormLayout title={t("calendar:forms.marketAdd.step2.title")}>
      <BaseCard title={headerContent} footer={footerButtons} className="w-full">
        <Form {...form}>
          <div className="space-y-4">
            <DetailRow
              label={t("calendar:forms.marketAdd.step1.opa")}
              value={
            ...
            <DetailRow
              label={t("calendar:forms.marketAdd.step1.startDate")}
            ...
            <DetailRow
              label={t("calendar:forms.marketAdd.step1.endDate")}
            ...
            <DetailRow
              label={t("calendar:forms.marketAdd.step1.eventTime")}
              value={eventTime || "-"}
              showChangeButton={true}
              changeHref={changeHref}
            />
            ...
            <DetailRow
              label={t("calendar:forms.marketAdd.step1.locationCode")}
              value={
            ...
            <DetailRow
              label={t("calendar:forms.marketAdd.step1.location")}
              value={location || "-"}
              showChangeButton={true}
              changeHref={changeHref}
              noBorder={true}
            ...
            <div className="mt-8 mb-6">
              <div className="flex items-center space-x-2">
                ...
                >
                  {t("calendar:forms.marketAdd.step2.confirmMessage")}
                </label>
              </div>

              {!isConfirmed && (
                <p className="text-sm text-red-600 mt-2">
                  {t("calendar:forms.marketAdd.step2.confirmRequired")}
                </p>
              )}
            </div>
          </div>
