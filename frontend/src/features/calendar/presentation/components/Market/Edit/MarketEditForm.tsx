...
export function MarketEditForm() {
  ...
  // Form setup
  const form = useForm<MarketEditData>({
    resolver: zodResolver(createMarketEditSchema(t)),
    defaultValues: {
      opaId: "",
      startDate: "",
      endDate: "",
    ...
    const loadOfflineData = async () => {
      ...
      try {
        ...
        if (pendingOperation && pendingOperation.payload) {
          ...
          const offlineData: MarketEditData = {
  ...
  const headerContent = (
    <div className="flex items-center space-x-3">
      <h1 className="text-xl font-medium text-gray-900">
        {t("calendar:forms.marketAdd.editTitle")}
      </h1>
    </div>
  );
