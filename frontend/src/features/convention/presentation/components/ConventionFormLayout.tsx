...
interface ConventionFormLayoutProps {
...
export function ConventionFormLayout({
  children,
  className,
  onHandleCancel,
  title,
}: ConventionFormLayoutProps) {
  ...
  const handleCancel = () => {
    if (hasUnsavedChanges) {
      const confirmLeave = window.confirm(t("form.unsavedChangesWarning"));
    ...
    <div className="min-h-screen bg-[#f8f9fa]">
      <PageHeader
        ...
        rightContent={[
          ...
          >
            <Icon name="X" />
            <span className="hidden lg:block">{t("form.cancelButton")}</span>
          </Button>,
        ]}
      />
