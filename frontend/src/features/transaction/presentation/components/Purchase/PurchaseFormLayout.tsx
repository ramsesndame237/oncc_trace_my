...
export function PurchaseFormLayout({
  children,
  className,
  onHandleCancel,
  title,
  ...
  const pageTitle =
    title ||
    (isEditMode
      ? t("transaction:purchaseAdd.editTitle")
      : t("transaction:purchaseAdd.createTitle"));

  const handleCancel = () => {
    if (hasUnsavedChanges) {
      const confirmLeave = window.confirm(
        t("transaction:purchaseAdd.messages.unsavedChanges")
      );
