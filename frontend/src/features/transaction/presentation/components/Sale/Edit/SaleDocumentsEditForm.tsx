...
export function SaleDocumentsEditForm() {
  ...
  const validationSchema = createStep3DocumentsSchema(t);
  ...
  const transactionTitle = transaction
    ? `${t("transaction:view.title")} - ${transaction.seller?.familyName || ""} ${
    ...
    const loadDocuments = async () => {
      ...
      try {
        ...
        for (const doc of result.documents) {
          ...
          if (saleContractDocTypes.has(docType)) {
  ...
  const handleSubmit = useCallback(
    async (data: Step3DocumentsData) => {
      ...
      try {
        ...
        const allDocuments = [...(data.saleContractDocuments || [])];
